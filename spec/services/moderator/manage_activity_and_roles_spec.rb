...
RSpec.describe(Moderator::ManageActivityAndRoles, type: :service) do
  ...
  it("updates user status") do
    ...
    user.reload
    described_class.handle_user_roles(admin: admin, user: user, user_params: {
      note_for_current_role: "warning user",
      user_status: "Warn",
    })
  ...
  it("updates negative role to positive role") do
    ...
    described_class.handle_user_roles(admin: admin, user: user, user_params: {
      note_for_current_role: "user in good standing",
      user_status: "Regular Member",
    })
    expect(user.banned).to(be(false))
  ...
  it("removes mentorship ban") do
    ...
    described_class.handle_user_roles(admin: admin, user: user, user_params: {
      toggle_mentorship: "0",
      mentorship_note: "Add to mentorship program",
    })
