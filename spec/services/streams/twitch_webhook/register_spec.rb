...
RSpec.describe(Streams::TwitchWebhook::Register, type: :service) do
  describe("::call") do
    ...
    let!(:twitch_webhook_registration_stubbed_route) do
      stub_request(:post, "https://api.twitch.tv/helix/webhooks/hub").with(body: expected_twitch_webhook_params, headers: expected_headers).and_return(status: 204)
    end

    let(:expected_twitch_user_params) {
      {
        login: "test-username",
      }
    }
    ...
    let!(:twitch_user_stubbed_route) do
      stub_request(:get, "https://api.twitch.tv/helix/users").with(query: expected_twitch_user_params, headers: expected_headers).and_return(body: {
        data: [{
          id: 654_321,
        }],
      }.to_json, headers: {
