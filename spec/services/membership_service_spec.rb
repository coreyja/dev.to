...
RSpec.describe(MembershipService) do
  ...
  def valid_instance(user = user_one, amount = 1200)
    customer = Stripe::Customer.create(email: "stripe_tester@dev.to", source: stripe_helper.generate_card_token)
  ...
  describe("#initialize") do
    ...
    context("when evoked on a user that already has subscription") do
      it("returns an object with subscription") do
        customer = Stripe::Customer.create(email: "stripe_tester@dev.to", source: stripe_source_token)
        user_one.update(stripe_id_code: customer.id)
        plan = Stripe::Plan.create(id: "membership-1200", currency: "usd", interval: "month", name: "Monthly DEV Membership", amount: 1200, statement_descriptor: "DEV membership")
