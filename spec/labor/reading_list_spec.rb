...
RSpec.describe(ReadingList) do
  ...
  def create_reaction(user, article)
    Reaction.create!(user_id: user.id, reactable_id: article.id, reactable_type: "Article", category: "readinglist")
  end
  ...
  it("returns cached ids of articles that have been reacted to") do
    create_reaction(user, article)
    create_reaction(user, article2)
    expect(described_class.new(user).cached_ids_of_articles).to(eq([
      article2.id,
      article.id,
    ]))
  end
