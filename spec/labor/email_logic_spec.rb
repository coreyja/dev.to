  ...
  describe("#analyze") do
    ...
    context("when a user's open_percentage is low ") do
      before do
        ...
        10.times do
          Ahoy::Message.create(mailer: "DigestMailer#digest_email", user_id: user.id, sent_at: Time.current.utc)
        end
      end
    ...
    context("when a user's open_percentage is high") do
      before do
        10.times do
          Ahoy::Message.create(mailer: "DigestMailer#digest_email", user_id: user.id, sent_at: Time.current.utc, opened_at: Time.current.utc)
          author = create(:user)
          user.follow(author)
