...
RSpec.describe(MarkdownParser) do
  ...
  context("when using gifs from Giphy as images") do
    let(:giphy_markdown_texts) do
      %w[![source](https://media.giphy.com/media/3ow0TN2M8TH2aAn67F/giphy.gif) ![social](https://media.giphy.com/media/3ow0TN2M8TH2aAn67F/giphy.gif) ![small](https://media.giphy.com/media/3ow0TN2M8TH2aAn67F/200w_d.gif)]
    end
  ...
  context("when an image is used") do
    ...
    it("wraps the image with Cloudinary") do
      expect(generate_and_parse_markdown(markdown_with_img)).to(include("https://res.cloudinary.com"))
    end
  end
