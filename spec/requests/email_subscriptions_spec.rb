...
RSpec.describe("EmailSubscriptions", type: :request) do
  ...
  def generate_token(user_id)
    Rails.application.message_verifier(:unsubscribe).generate(user_id: user_id, email_type: :email_mention_notifications, expires_at: 31.days.from_now)
  end
  ...
  describe("GET /email_subscriptions/unsubscribe") do
    ...
    it("handles error properly") do
      expect { get(email_subscriptions_unsubscribe_url) }.to(raise_error(ActiveRecord::RecordNotFound))
    end
