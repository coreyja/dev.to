...
RSpec.describe("ImageUploads", type: :request) do
  describe("POST/image_uploads") do
    ...
    let(:image) do
      Rack::Test::UploadedFile.new(Rails.root.join("spec", "support", "fixtures", "images", "image1.jpeg"), "image/jpeg")
    end

    let(:bad_image) do
      Rack::Test::UploadedFile.new(Rails.root.join("spec", "support", "fixtures", "images", "bad-image.jpg"), "image/jpeg")
    end
    ...
    context("when logged-in") do
      ...
      it("provides a link") do
        post("/image_uploads", headers: headers, params: {
          image: image,
        })
