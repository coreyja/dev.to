  ...
  describe("GET /reactions?article_id=:article_id") do
    context("when signed in") do
      ...
      it("does not set Fastly cache control and surrogate control headers") do
        expect(response.headers.to_hash).not_to(include("Cache-Control" => "public, no-cache", "Surrogate-Control" => "max-age=86500, stale-if-error=26400"))
      end
    end
    ...
    context("when signed out") do
      ...
      it("sets Fastly cache control and surrogate control headers") do
        expect(response.headers.to_hash).to(include("Cache-Control" => "public, no-cache", "Surrogate-Control" => "max-age=86500, stale-if-error=26400"))
      end
    end
  end
