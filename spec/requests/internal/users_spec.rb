...
RSpec.describe("Internal::Users", type: :request) do
  ...
  def full_profile
    BadgeAchievement.create(user_id: user2.id, badge_id: badge.id, rewarding_context_message_markdown: "message")
  ...
  context("when merging users") do
    ...
    it("deletes duplicate user") do
      post("/internal/users/#{user.id}/merge", params: {
        user: {
          merge_user_id: user2.id,
        },
      })
  ...
  context("when managing activty and roles") do
    it("adds comment ban role") do
      patch("/internal/users/#{user.id}/user_status", params: {
        user: {
          user_status: "Comment Ban",
          note_for_current_role: "comment ban this user",
        },
      })
    ...
    it("creates a general note on the user") do
      put("/internal/users/#{user.id}", params: {
        user: {
          new_note: "general note about whatever",
        },
      })
  ...
  context("when deleting user") do
    def create_mention
      comment = create(:comment, body_markdown: "Hello @#{user.username}, you are cool.", user_id: user2.id, commentable_id: article2.id)
