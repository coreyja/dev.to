...
RSpec.describe("TwitchStramUpdates", type: :request) do
  ...
  describe("POST /users/:user_id/twitch_stream_updates") do
    context("when the user was not streaming and starts streaming") do
      ...
      it("updates the Users twitch streaming status") do
        expect {
          post("/users/#{user.id}/twitch_stream_updates", params: twitch_webhook_params.to_json, headers: {
            "Content-Type" => "application/json",
            "X-Hub-Signature" => twitch_webhook_secret_sha,
          })
        }.to(change { user.reload.currently_streaming? }.from(false).to(true).and(change { user.reload.currently_streaming_on_twitch? }.from(false).to(true)))
      end
    end
    ...
    context("when the webhook secret was NOT verified") do
      ...
      it("noops") do
        ...
        }.to(not_change { user.reload.currently_streaming? }.from(false).and(not_change { user.reload.currently_streaming_on_twitch? }.from(false)))
      end
    end
  end
  ...
  context("when the user was streaming and stops") do
    ...
    it("updates the Users twitch streaming status") do
      ...
      }.to(change { user.reload.currently_streaming? }.from(true).to(false).and(change { user.reload.currently_streaming_on_twitch? }.from(true).to(false)))
    end
  end
end
