...
RSpec.describe("internal/users", type: :request) do
  ...
  describe("PUT internal/users/:id") do
    it("pairs mentor with a mentee") do
      put("/internal/users/#{mentor.id}", params: {
        user: {
          add_mentee: mentee.id,
        },
      })
    ...
    it("pairs mentee with a mentor") do
      put("/internal/users/#{mentee.id}", params: {
        user: {
          add_mentor: mentor.id,
        },
      })
    ...
    it("deactivates existing mentorships when user is banned") do
      ...
      patch("/internal/users/#{mentor.id}/user_status", params: {
        user: {
          toggle_mentorship: "1",
          mentorship_note: "banned from mentorship",
        },
      })
