...
RSpec.describe(HtmlCssToImage) do
  describe(".url") do
    it("returns the url to the created image") do
      stub_request(:post, /hcti.io/).to_return(status: 200, body: "{ \"url\": \"https://hcti.io/v1/image/6c52de9d-4d37-4008-80f8-67155589e1a1\" }", headers: {
    ...
    it("returns fallback image if the request fails") do
      stub_request(:post, /hcti.io/).to_return(status: 429, body: "{ \"error\": \"Plan limit exceeded\" }", headers: {
        "Content-Type" => "application/json",
      })
