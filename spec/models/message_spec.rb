...
RSpec.describe(Message, type: :model) do
  ...
  it("is valid if under 1024 chars") do
    message = build(:message, chat_channel_id: chat_channel.id, user_id: user.id, message_markdown: "hello")
    expect(message).to(be_valid)
  end

  it("creates rich link in connect with proper link") do
    article = create(:article)
    message = create(:message, chat_channel_id: chat_channel.id, user_id: user.id, message_markdown: "hello http://#{ApplicationConfig["APP_DOMAIN"]}#{article.path}")
  ...
  it("creates rich link in connect with non-rich link") do
    message = create(:message, chat_channel_id: chat_channel.id, user_id: user.id, message_markdown: "hello http://#{ApplicationConfig["APP_DOMAIN"]}/report-abuse")
    expect(message.message_html).not_to(include("data-content"))
  end

  it("sends email if user not recently active on /connect") do
    chat_channel.add_users([
      user,
      user2,
    ])
    ...
    create(:message, chat_channel_id: chat_channel.id, user_id: user.id, message_markdown: "hello http://#{ApplicationConfig["APP_DOMAIN"]}/report-abuse")
