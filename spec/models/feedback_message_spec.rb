...
RSpec.describe(FeedbackMessage, type: :model) do
  ...
  describe("validations for an abuse report") do
    subject(:feedback_message) do
      FeedbackMessage.new(feedback_type: "abuse-reports", reported_url: "https://dev.to", category: "spam", message: "something")
    end
    ...
    it do
      expect(feedback_message).to(validate_inclusion_of(:category).in_array([
        "spam",
        "other",
        "rude or vulgar",
        "harassment",
  ...
  describe("validations for a bug report") do
    subject(:feedback_message) do
      FeedbackMessage.new(feedback_type: "bug-reports", category: "bug", message: "something")
    end
