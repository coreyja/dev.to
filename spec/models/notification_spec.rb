...
RSpec.describe(Notification, type: :model) do
  ...
  describe("#send_new_follower_notification") do
    ...
    context("when a user follows another user") do
      ...
      it("creates a notification from the follow instance") do
        notifiable_data = {
          notifiable_id: Notification.first.notifiable_id,
          notifiable_type: Notification.first.notifiable_type,
        }
        follow_data = {
          notifiable_id: follow_instance.id,
          notifiable_type: follow_instance.class.name,
        }
  ...
  describe("#send_reaction_notification") do
    ...
    it("creates positive reaction notification") do
      reaction = Reaction.create!(user_id: user2.id, reactable_id: article.id, reactable_type: "Article", category: "like")
    ...
    it("does not create negative notification") do
      ...
      reaction = Reaction.create!(user_id: user2.id, reactable_id: article.id, reactable_type: "Article", category: "vomit")
      notification = Notification.send_reaction_notification_without_delay(reaction, reaction.reactable.user)
  ...
  describe("#send_milestone_notification") do
    let(:view_milestone_hash) {
      {
        type: "View",
        article: article,
      }
    }
    let(:reaction_milestone_hash) {
      {
        type: "Reaction",
        article: article,
      }
