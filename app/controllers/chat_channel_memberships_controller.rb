class ChatChannelMembershipsController < ApplicationController
  ...
  def create
    ...
    ChatChannelMembership.create(user_id: permitted_params[:user_id], chat_channel_id: @chat_channel.id, status: "pending")
    @chat_channel.index!
  end
  ...
  def update
    ...
    @chat_channels_memberships = current_user.chat_channel_memberships.includes(:chat_channel).where(status: "pending").order("chat_channel_memberships.updated_at DESC")
    render("chat_channels/index.json")
  end

  def destroy
    @chat_channel_membership = ChatChannel.find(params[:id]).chat_channel_memberships.where(user_id: current_user.id).first
    authorize(@chat_channel_membership)
