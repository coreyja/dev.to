class UsersController < ApplicationController
  ...
  def remove_association
    ...
    if @user.identities.size == 2 && identity
      ...
      redirect_to("/settings/#{@tab}", notice: "Your #{provider.capitalize} account was successfully removed.")
    else
  ...
  def join_org
    ...
    if (@organization = Organization.find_by(secret: params[:org_secret]))
      ...
      redirect_to("/settings/organization", notice: "You have joined the #{@organization.name} organization.")
    else
      not_found
    end
  end
  ...
  def leave_org
    ...
    redirect_to("/settings/organization", notice: "You have left your organization.")
  end
  ...
  def add_org_admin
    ...
    org_membership.save
    redirect_to("/settings/organization", notice: "#{user.name} is now an admin.")
  end
  ...
  def remove_org_admin
    ...
    org_membership.save
    redirect_to("/settings/organization", notice: "#{user.name} is no longer an admin.")
  end
  ...
  def remove_from_org
    ...
    redirect_to("/settings/organization", notice: "#{user.name} is no longer part of your organization.")
  end
  ...
  def handle_settings_tab
    ...
    when "integrations"
      ...
      if current_user.identities.where(provider: "github").any?
        @client = Octokit::Client.new(access_token: current_user.identities.where(provider: "github").last.token)
      end
