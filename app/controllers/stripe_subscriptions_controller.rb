class StripeSubscriptionsController < ApplicationController
  ...
  def create
    ...
    if customer && MembershipService.new(customer, current_user, amount).subscribe_customer
      ...
      redirect_to("/membership", flash: {
        error: "There was a problem updating your billing info.",
      })
    end
  ...
  def update
    ...
    if MembershipService.new(customer, current_user, amount).update_subscription
      ...
      redirect_to("/settings/membership", notice: "Your new membership is now active. Thanks for your support!")
    else
      logger.error("Stripe Update Subscription Failure - #{current_user.username}")
      redirect_to("/settings/membership", flash: {
        error: "There was a problem updating your new plan.",
      })
    end

  rescue Stripe::CardError, Stripe::InvalidRequestError => e
    redirect_to("/settings/billing", flash: {
      error: e.message,
    })
  end
  ...
  def destroy
    ...
    if MembershipService.new(customer, current_user, nil).unsubscribe_customer
      ...
      redirect_to("/settings", notice: "Your membership is cancelled. Thanks for your support.")
    else
      logger.info("Stripe Cancel Subscription Failure - #{current_user.username}")
      redirect_to("/settings", flash: {
        error: "There was a problem updating your plan.",
      })
    end
  end
  ...
  def find_or_create_customer
    if current_user.stripe_id_code.present?
      ...
      customer = Stripe::Customer.create(email: current_user.email, source: params[:stripe_token])
    end
...
HTML
    Stripe::CardError.new(message.html_safe, params[:amount], 400)
  end
