class FollowedArticlesController < ApplicationController
  ...
  caches_action(:index, cache_path: proc { "followed_articles_#{current_user.id}__#{current_user.updated_at}__#{user_signed_in?}" }, expires_in: 35.minutes)
  def index
    @articles = if current_user
      Rails.cache.fetch("user-#{current_user.id}__#{current_user.updated_at}/followed_articles", expires_in: 30.minutes) do
        current_user.followed_articles.includes(:user).where("published_at > ?", 5.days.ago).order("hotness_score DESC").limit(25).map do |a|
  ...
  def inappropriate_hiring_instance(article)
    (article.decorate.cached_tag_list_array.include?("hiring") && !article.approved) || (article.decorate.cached_tag_list_array.include?("hiring") && current_user.cached_followed_tag_names.exclude?("hiring"))
  end
