class ChatChannelsController < ApplicationController
  ...
  def create
    ...
    if @chat_channel.valid?
      ...
      render(json: {
        errors: @chat_channel.errors.full_messages,
      })
    end
  end
  ...
  def moderate
    ...
    when "/ban"
      ...
      if banned_user
        ...
        Pusher.trigger(@chat_channel.pusher_channels, "user-banned", {
  ...
  def create_chat
    ...
    if chat_recipient.inbox_type == "open"
      chat = ChatChannel.create_with_users([
        current_user,
        chat_recipient,
      ], "direct")
      ...
      message = Message.new(chat_channel: chat, message_markdown: message_markdown, user: current_user)
  ...
  def render_unopened_json_response
    @chat_channels_memberships = if current_user
      current_user.chat_channel_memberships.includes(:chat_channel).where("has_unopened_messages = ? OR status = ?", true, "pending").where(show_global_badge_notification: true).order("chat_channel_memberships.updated_at DESC")
    else
      []
    end
  ...
  def render_pending_json_response
    @chat_channels_memberships = if current_user
      current_user.chat_channel_memberships.includes(:chat_channel).where(status: "pending").order("chat_channel_memberships.updated_at DESC")
    else
      []
    end
  ...
  def generate_algolia_search_key
    ...
    params = {
      filters: "viewable_by:#{current_user_id} AND status: active",
    }
    @secured_algolia_key = Algolia.generate_secured_api_key(ApplicationConfig["ALGOLIASEARCH_SEARCH_ONLY_KEY"], params)
  end
