class CommentsController < ApplicationController
  ...
  def index
    ...
    if @podcast
      ...
      @user = User.find_by(username: params[:username]) || Organization.find_by(slug: params[:username]) || not_found
      @commentable = @root_comment&.commentable || @user.articles.find_by(slug: params[:slug]) || not_found
      @article = @commentable
  ...
  def create
    ...
    elsif (@comment = Comment.where(body_markdown: @comment.body_markdown, commentable_id: @comment.commentable.id, ancestry: @comment.ancestry)[1])
      @comment.destroy
      render(json: {
        status: "comment already exists",
      })
