module Api
  module V0
    class ArticlesController < ApiController
      ...
      caches_action(:show, cache_path: proc { |c| c.params.permit! }, expires_in: 5.minutes)
      respond_to(:json)
      ...
      def onboarding
        ...
        Article.tagged_with(tag_list, any: true).order("published_at DESC").where("positive_reactions_count > ? OR comments_count > ? AND published = ?", 10, 3, true).limit(15).each do |article|
          @articles << article
        end
      ...
      def article_params
        ...
        params.require(:article).permit(:title, :body_markdown, :main_image, :published, :description, :tag_list, :organization_id, :canonical_url, :series, :collection_id)
