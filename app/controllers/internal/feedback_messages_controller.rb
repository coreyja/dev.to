class Internal::FeedbackMessagesController < Internal::ApplicationController
  ...
  def index
    ...
    @feedback_messages = FeedbackMessage.where(feedback_type: @feedback_type, status: @status).includes(:reporter, :notes).order("feedback_messages.created_at DESC").page(params[:page] || 1).per(5)
  ...
  def save_status
    ...
    if feedback_message.update(status: params[:status])
      ...
      render(json: {
        outcome: feedback_message.errors.full_messages,
      })
    end
  end
  ...
  def create_note
    note = Note.new(noteable_id: params["noteable_id"], noteable_type: params["noteable_type"], author_id: params["author_id"], content: params["content"], reason: params["reason"])
    ...
    if note.save
      ...
      render(json: {
        outcome: note.errors.full_messages,
      })
    end
  end
  ...
  def send_slack_message(params)
    SlackBot.ping(generate_message(params), channel: params["feedback_type"], username: "new_note_bot", icon_emoji: ":memo:")
  end
...
Message: #{params["content"]}
  ...
  def feedback_message_params
    params[:feedback_message].permit(:id, :status, :reviewer_id, note: %i[content reason noteable_id noteable_type author_id])
  end
end
