class StoriesController < ApplicationController
  ...
  def redirect_to_changed_username_profile
    ...
    user_or_org = User.find_by("old_username = ? OR old_old_username = ?", potential_username, potential_username) || Organization.find_by("old_slug = ? OR old_old_slug = ?", potential_username, potential_username)
  ...
  def handle_base_index
    ...
    if %w[week month year infinity].include?(params[:timeframe])
      @stories = @stories.where("published_at > ?", Timeframer.new(params[:timeframe]).datetime).order("score DESC")
    elsif params[:timeframe] == "latest"
      @stories = @stories.order("published_at DESC").where("featured_number > ? AND score > ?", 1_449_999_999, -40)
      @featured_story = Article.new
    else
      @default_home_feed = true
      @stories = @stories.where("score > ? OR featured = ?", 9, true).order("hotness_score DESC")
  ...
  def handle_organization_index
    @user = @organization
    @stories = ArticleDecorator.decorate_collection(@organization.articles.published.limited_column_select.order("published_at DESC").page(@page).per(8))
    @article_index = true
  ...
  def handle_user_index
    ...
    @stories = ArticleDecorator.decorate_collection(@user.articles.published.limited_column_select.order("published_at DESC").page(@page).per(user_signed_in? ? 2 : 5))
    @article_index = true
    @list_of = "articles"
  ...
  def stories_by_timeframe
    if %w[week month year infinity].include?(params[:timeframe])
      @stories.where("published_at > ?", Timeframer.new(params[:timeframe]).datetime).order("positive_reactions_count DESC")
    elsif params[:timeframe] == "latest"
  ...
  def assign_podcasts
    ...
    @podcast_episodes = PodcastEpisode.includes(:podcast).order("published_at desc").select(:slug, :title, :podcast_id).limit(5)
  end

  def article_finder(num_articles)
    tag = params[:tag]
    articles = Article.published.limited_column_select.page(@page).per(num_articles)
    articles = articles.cached_tagged_with(tag) if tag.present?
    articles
  end
end
