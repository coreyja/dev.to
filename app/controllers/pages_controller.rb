class PagesController < ApplicationController
  ...
  def report_abuse
    ...
    @feedback_message = FeedbackMessage.new(reported_url: reported_url&.chomp("?i=i"))
  ...
  def live
    ...
    @chat_channels = [@active_channel].to_json(only: %i[channel_name channel_type last_message_at slug status id])
  end

  def shecoded
    @top_articles = Article.published.tagged_with(%w[shecoded shecodedally theycoded], any: true).where(approved: true).where("published_at > ? AND score > ?", 3.weeks.ago, 28).order(Arel.sql("RANDOM()")).includes(:user).decorate
    @articles = Article.published.tagged_with(%w[shecoded shecodedally theycoded], any: true).where(approved: true).where("published_at > ? AND score > ?", 3.weeks.ago, -8).order(Arel.sql("RANDOM()")).where.not(id: @top_articles.pluck(:id)).includes(:user).decorate
    render(layout: false)
  ...
  def members_for_display
    Rails.cache.fetch("members-for-display-on-membership-page", expires_in: 6.hours) do
      roles = %i[level_1_member level_2_member level_3_member level_4_member triple_unicorn_member workshop_pass]
