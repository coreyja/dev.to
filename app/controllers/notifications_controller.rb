class NotificationsController < ApplicationController
  ...
  def filtered_notifications
    if params[:filter].to_s.casecmp("posts").zero?
      Notification.where(user_id: @user.id, notifiable_type: "Article", action: "Published").order("notified_at DESC")
    elsif params[:filter].to_s.casecmp("comments").zero?
      Notification.where(user_id: @user.id, notifiable_type: "Comment", action: nil).or(Notification.where(user_id: @user.id, notifiable_type: "Mention")).order("notified_at DESC")
    end
  end

  def organization_notifications
    if params[:filter].to_s.casecmp("comments").zero?
      Notification.where(organization_id: params[:org_id], notifiable_type: "Comment", action: nil, user_id: nil).or(Notification.where(organization_id: params[:org_id], notifiable_type: "Mention", user_id: nil)).order("notified_at DESC")
    else
      Notification.where(organization_id: params[:org_id], user_id: nil).order("notified_at DESC")
    end
  end
