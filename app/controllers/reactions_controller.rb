class ReactionsController < ApplicationController
  ...
  def index
    ...
    if params[:article_id]
      ...
      reactions = if efficient_current_user_id.present?
        Reaction.where(reactable_id: id, reactable_type: "Article", user_id: efficient_current_user_id).where("points > ?", 0)
      else
        []
      end
      ...
      render(json: {
        current_user: {
          id: efficient_current_user_id,
        },
        article_reaction_counts: Reaction.count_for_article(id),
        reactions: reactions,
      }.to_json)
    else
      comments = Comment.where(commentable_id: params[:commentable_id], commentable_type: params[:commentable_type]).select(%i[id positive_reactions_count])
      comment_ids = comments.map(&:id)
      reaction_counts = comments.map { |c|
        {
          id: c.id,
          count: c.positive_reactions_count,
        }
      }
  ...
  def create
    ...
    reaction = Reaction.where(user_id: current_user.id, reactable_id: params[:reactable_id], reactable_type: params[:reactable_type], category: category).first
    ...
    if reaction
      ...
      reaction = Reaction.create!(user_id: current_user.id, reactable_id: params[:reactable_id], reactable_type: params[:reactable_type], category: category)
      @result = "create"
  ...
  def cached_user_positive_reactions(user)
    Rails.cache.fetch("cached_user_reactions-#{user.id}-#{user.updated_at}", expires_in: 24.hours) do
      Reaction.where(user_id: user.id).where("points > ?", 0)
    end
  end
