...
if (postscribe) {
  ...
  def parse_link(link)
    ...
    if valid_link?(input_no_space)
      ...
      raise StandardError, "Invalid Gist link: #{link} Links must follow this format: https://gist.github.com/username/gist_id"
    end
  end
  ...
  def valid_link?(link)
    (link =~ /\Ahttps\:\/\/gist\.github\.com\/([a-zA-Z0-9](-?[a-zA-Z0-9]){0,38})\/([a-zA-Z0-9]){1,32}\Z/)&.zero?
  end
