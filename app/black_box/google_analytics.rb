...
class GoogleAnalytics
  ...
  def get_feed_impression_info
    requests = @article_ids.map do |id|
      make_report_request("ga:eventAction==featured-feed-impression;ga:eventLabel==articles-#{id}", "ga:totalEvents")
    end
  ...
  def get_feed_click_info
    requests = @article_ids.map do |id|
      make_report_request("ga:eventAction==featured-feed-click;ga:eventLabel==articles-#{id}", "ga:totalEvents")
    end
  ...
  def make_report_request(filters_expression_string, metrics_string)
    ReportRequest.new(view_id: ApplicationConfig["GA_VIEW_ID"], filters_expression: filters_expression_string, metrics: [Metric.new(expression: metrics_string)], dimensions: [Dimension.new(name: "ga:segment")], segments: [
      Segment.new(segment_id: "gaid::-1"),
  ...
  def create_service_account_credential
    ServiceAccountCredentials.make_creds(json_key_io: OpenStruct.new(read: ApplicationConfig["GA_SERVICE_ACCOUNT_JSON"]), scope: [AUTH_ANALYTICS_READONLY])
  end
end
