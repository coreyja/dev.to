class RateLimitChecker
  ...
  def limit_by_email_recipient_address(address)
    ...
    EmailMessage.where(to: address).where("sent_at > ?", 2.minutes.ago).size > 5
  end

  def ping_admins
    return unless user && Rails.env.production?
    SlackBot.ping("Rate limit exceeded. https://dev.to#{user.path}", channel: "abuse-reports", username: "rate_limit", icon_emoji: ":hand:")
  end
