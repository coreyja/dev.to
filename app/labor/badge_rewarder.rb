module BadgeRewarder
  ...
  def self.award_yearly_club_badges
    ...
    (1..3).each do |i|
      ...
      User.where("created_at < ? AND created_at > ?", i.year.ago, i.year.ago - 2.days).find_each do |user|
        achievement = BadgeAchievement.create(user_id: user.id, badge_id: badge.id, rewarding_context_message_markdown: message)
  ...
  def self.award_beloved_comment_badges
    ...
    Comment.where("positive_reactions_count > ?", 24).find_each do |comment|
      ...
      achievement = BadgeAchievement.create(user_id: comment.user_id, badge_id: Badge.find_by(slug: "beloved-comment")&.id || 3, rewarding_context_message_markdown: message)
  ...
  def self.award_contributor_badges_from_github(since = 1.day.ago, message_markdown = "Thank you so much for your contributions!")
    ...
    [
      ...
      Identity.where(provider: "github", uid: authors_uids).find_each do |i|
        BadgeAchievement.where(user_id: i.user_id, badge_id: badge.id).first_or_create(rewarding_context_message_markdown: message_markdown)
      end
    end
  end

  def self.award_streak_badge(num_weeks)
    article_user_ids = Article.published.where("published_at > ? AND score > ?", 1.week.ago, -25).pluck(:user_id)
  ...
  def self.award_badges(usernames, slug, message_markdown)
    ...
    User.where(username: usernames).find_each do |user|
      BadgeAchievement.create(user_id: user.id, badge_id: Badge.find_by(slug: slug).id, rewarding_context_message_markdown: message_markdown)
      user.save
    end
  end
end
