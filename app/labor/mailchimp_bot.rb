class MailchimpBot
  ...
  def upsert_to_newsletter
    ...
    success = false
    begin
      gibbon.lists(ApplicationConfig["MAILCHIMP_NEWSLETTER_ID"]).members(target_md5_email).upsert(body: {
  ...
  def manage_community_moderator_list
    ...
    begin
      gibbon.lists(ApplicationConfig["MAILCHIMP_COMMUNITY_MODERATORS_ID"]).members(target_md5_email).upsert(body: {
        email_address: user.email,
        status: status,
        merge_fields: {
          NAME: user.name.to_s,
  ...
  def manage_tag_moderator_list
    ...
    begin
      gibbon.lists(ApplicationConfig["MAILCHIMP_TAG_MODERATORS_ID"]).members(target_md5_email).upsert(body: {
        email_address: user.email,
        status: status,
        merge_fields: {
          NAME: user.name.to_s,
  ...
  def upsert_to_membership_newsletter
    ...
    begin
      gibbon.lists(ApplicationConfig["MAILCHIMP_SUSTAINING_MEMBERS_ID"]).members(target_md5_email).upsert(body: {
        email_address: user.email,
        status: status,
        merge_fields: {
          NAME: user.name.to_s,
  ...
  def unsub_sustaining_member
    ...
    gibbon.lists(ApplicationConfig["MAILCHIMP_TAG_MODERATORS_ID"]).members(target_md5_email).update(body: {
      status: "unsubscribed",
    })
  end
  ...
  def unsub_community_mod
    ...
    gibbon.lists(ApplicationConfig["MAILCHIMP_COMMUNITY_MODERATORS_ID"]).members(target_md5_email).update(body: {
      status: "unsubscribed",
    })
  end
  ...
  def unsub_tag_mod
    ...
    gibbon.lists(ApplicationConfig["MAILCHIMP_SUSTAINING_MEMBERS_ID"]).members(target_md5_email).update(body: {
      status: "unsubscribed",
    })
  end
  ...
  def unsubscribe_all_newsletters
    success = false
    begin
      gibbon.lists(ApplicationConfig["MAILCHIMP_NEWSLETTER_ID"]).members(target_md5_email).update(body: {
        status: "unsubscribed",
      })
      unsub_tag_mod
