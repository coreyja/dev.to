module Exporter
  class Comments
    ...
    def allowed_attributes
      %i[body_markdown created_at deleted edited edited_at id_code markdown_character_count positive_reactions_count processed_html receive_notifications]
    end
    ...
    def jsonify(comments)
      ...
      comments.includes(:commentable).select(attributes_to_select).find_each do |comment|
        ...
        comments_to_jsonify << comment.as_json(only: allowed_attributes).merge(commentable_path: comment.commentable&.path)
      end
