  ...
  module Users
    class Recent
      ...
      def tagged_article_user_ids(num_weeks = 1)
        Article.published.tagged_with(user.decorate.cached_followed_tag_names, any: true).where("score > ? AND published_at > ?", article_reaction_count, num_weeks.weeks.ago).pluck(:user_id).each_with_object(Hash.new(0)) { |value, counts| counts[value] += 1 }.sort_by { |_key, value| value }.map { |arr| arr[0] }
      end
      ...
      def recent_top_producers
        User.where("articles_count > ? AND comments_count > ?", established_user_article_count, established_user_comment_count).order("updated_at DESC").limit(50).to_a
      end
