class RssReader
  ...
  def create_articles_for_user(user)
    ...
    feed.entries.reverse_each do |item|
      ...
      log_error("RssReaderError: occurred while creating article", rss_reader_info: {
        user: user.username,
    ...
    log_error("RssReaderError: occurred while fetching feed", rss_reader_info: {
      user: user.username,
      feed_url: user.feed_url,
      item_count: get_item_count_error(feed),
      error: e,
  ...
  def make_from_rss_item(item, user, feed)
    ...
    article = Article.create!(feed_source_url: feed_source_url, user_id: user.id, published_at: item.published, published_from_feed: true, show_comments: true, body_markdown: RssReader::Assembler.call(item, user, feed, feed_source_url), organization_id: user.organization_id.presence)
  ...
  def medium_reply?(item)
    get_host_without_www(item.url.strip) == "medium.com" && !item[:categories] && content_is_not_the_title?(item)
  end
  ...
  def send_slack_notification(article)
    ...
    SlackBot.delay.ping("New Article Retrieved via RSS: #{article.title}\nhttps://dev.to#{article.path}", channel: "activity", username: "article_bot", icon_emoji: ":robot_face:")
  end
