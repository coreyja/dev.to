  ...
  module NewFollower
    class Send
      ...
      def initialize(follow_data, is_read = false)
        ...
        @followable_id = follow_data.followable_id
        @followable_type = follow_data.followable_type
      ...
      def call
        recent_follows = Follow.where(followable_type: followable_type, followable_id: followable_id).where("created_at > ?", 24.hours.ago).order("created_at DESC")
        notification_params = {
          action: "Follow",
        }
