module Streams
  module TwitchWebhook
    class Register
      ...
      def call
        user_resp = HTTParty.get("https://api.twitch.tv/helix/users", query: {
          login: user.twitch_username,
        }, headers: authentication_request_headers)
        twitch_user_id = user_resp["data"].first["id"]
        HTTParty.post("https://api.twitch.tv/helix/webhooks/hub", body: webhook_request_body(twitch_user_id), headers: authentication_request_headers)
      end
