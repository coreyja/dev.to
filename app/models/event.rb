class Event < ApplicationRecord
  ...
  scope(:in_the_future_and_published, lambda { where("starts_at > ?", Time.current).where(published: true) })
  scope(:in_the_past_and_published, lambda { where("starts_at < ?", Time.current).where(published: true) })
  private
