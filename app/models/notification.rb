class Notification < ApplicationRecord
  ...
  validates(:user_id, uniqueness: {
    scope: %i[organization_id notifiable_id notifiable_type action],
  })
  class << self
    ...
    def send_to_followers(notifiable, action = nil)
      ...
      followers.sort_by(&:updated_at).reverse[0..10_000].each do |follower|
        Notification.create(user_id: follower.id, notifiable_id: notifiable.id, notifiable_type: notifiable.class.name, action: action, json_data: json_data)
      end
    end
    ...
    def send_mention_notification(mention)
      ...
      Notification.create(user_id: mention.user_id, notifiable_id: mention.id, notifiable_type: "Mention", action: nil, json_data: json_data)
    end
    ...
    def send_tag_adjustment_notification(tag_adjustment)
      ...
      Notification.create(user_id: article.user_id, notifiable_id: tag_adjustment.id, notifiable_type: tag_adjustment.class.name, json_data: json_data)
    ...
    def send_milestone_notification(milestone_hash)
      ...
      json_data = {
        article: article_data(milestone_hash[:article]),
        gif_id: RandomGif.new.random_id,
      }
      Notification.create!(user_id: milestone_hash[:article].user_id, notifiable_id: milestone_hash[:article].id, notifiable_type: "Article", json_data: json_data, action: "Milestone::#{milestone_hash[:type]}::#{milestone_hash[:next_milestone]}")
      return unless milestone_hash[:article].organization_id
      Notification.create!(organization_id: milestone_hash[:article].organization_id, notifiable_id: milestone_hash[:article].id, notifiable_type: "Article", json_data: json_data, action: "Milestone::#{milestone_hash[:type]}::#{milestone_hash[:next_milestone]}")
    end

    handle_asynchronously(:send_milestone_notification)
    def remove_all(notifiable_hash)
      Notification.where(notifiable_id: notifiable_hash[:notifiable_id], notifiable_type: notifiable_hash[:notifiable_type], action: notifiable_hash[:action]).delete_all
    end
    ...
    def remove_each(notifiable_collection, action = nil)
      ...
      notifiable_collection.each do |notifiable|
        Notification.where(notifiable_id: notifiable.id, notifiable_type: notifiable.class.name, action: action).destroy_all
      end
    end

    handle_asynchronously(:remove_each)
    def update_notifications(notifiable, action = nil)
      notifications = Notification.where(notifiable_id: notifiable.id, notifiable_type: notifiable.class.name, action: action)
    ...
    def should_send_milestone?(milestone_hash)
      ...
      last_milestone_notification = Notification.find_by(user_id: milestone_hash[:article].user_id, notifiable_type: "Article", notifiable_id: milestone_hash[:article].id, action: "Milestone::#{milestone_hash[:type]}::#{milestone_hash[:next_milestone]}")
