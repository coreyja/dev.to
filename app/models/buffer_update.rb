class BufferUpdate < ApplicationRecord
  ...
  validates(:status, inclusion: {
    in: %w[pending sent_direct confirmed dismissed],
  })
  def self.buff!(article_id, text, buffer_profile_id_code, social_service_name = "twitter", tag_id = nil)
    ...
    create(article_id: article_id, tag_id: tag_id, body_text: text, buffer_profile_id_code: buffer_profile_id_code, social_service_name: social_service_name, buffer_response: buffer_response, status: "sent_direct")
  end
  ...
  def self.send_to_buffer(text, buffer_profile_id_code)
    ...
    client.create_update(body: {
      text: text,
      profile_ids: [buffer_profile_id_code],
    })
  end
  ...
  def validate_body_text_recent_uniqueness
    ...
    if BufferUpdate.where(body_text: body_text, article_id: article_id, tag_id: tag_id, social_service_name: social_service_name).where("created_at > ?", 2.minutes.ago).any?
