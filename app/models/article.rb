class Article < ApplicationRecord
  ...
  validates(:slug, presence: {
    if: :published?,
  }, format: /\A[0-9a-z\-_]*\z/, uniqueness: {
    scope: :user_id,
  })
  ...
  validates(:feed_source_url, uniqueness: {
    allow_blank: true,
  })
  validates(:canonical_url, url: {
    allow_blank: true,
    no_local: true,
  ...
  validates(:video_state, inclusion: {
    in: %w[PROGRESSING COMPLETED],
  }, allow_nil: true)
  validates(:cached_tag_list, length: {
    maximum: 86,
  })
  validates(:main_image, url: {
    allow_blank: true,
    schemes: %w[https http],
  })
  ...
  validates(:video_source_url, url: {
    allow_blank: true,
    schemes: ["https"],
  })
  validates(:video_thumbnail_url, url: {
    allow_blank: true,
    schemes: %w[https http],
  })
  validates(:video_closed_caption_track_url, url: {
    allow_blank: true,
    schemes: ["https"],
  })
  validates(:video_source_url, url: {
  ...
  scope(:active_help, lambda { published.cached_tagged_with("help").order("created_at DESC").where("published_at > ? AND comments_count < ?", 12.hours.ago, 6) })
  ...
  algoliasearch(per_environment: true, auto_remove: false, enqueue: :trigger_delayed_index) do
    ...
    add_index("searchables", id: :index_id, per_environment: true, enqueue: :trigger_delayed_index) do
      attributes(:title, :tag_list, :main_image, :id, :reading_time, :score, :featured, :published, :published_at, :featured_number, :comments_count, :reactions_count, :positive_reactions_count, :path, :class_name, :user_name, :user_username, :comments_blob, :body_text, :tag_keywords_for_search, :search_score, :readable_publish_date, :flare_tag)
      ...
      tags do
        [
          ...
          "lang_#{language || "en"}",
          ("organization_#{organization_id}" if organization),
        ].flatten.compact
      end
    ...
    add_index("ordered_articles", id: :index_id, per_environment: true, enqueue: :trigger_delayed_index) do
      attributes(:title, :path, :class_name, :comments_count, :reading_time, :language, :tag_list, :positive_reactions_count, :id, :hotness_score, :score, :readable_publish_date, :flare_tag, :user_id, :organization_id, :cloudinary_video_url, :video_duration_in_minutes, :experience_level_rating, :experience_level_rating_distribution)
  ...
  def self.filter_excluded_tags(tag = nil)
    ...
    if tag == "hiring"
      ...
      tagged_with(tag).tagged_with("hiring", exclude: true)
    else
  ...
  def self.active_threads(tags = ["discuss"], time_ago = nil, number = 10)
    ...
    stories = if time_ago == "latest"
      ...
      stories.order("comments_count DESC").where("published_at > ? AND score > ?", time_ago, -5)
    else
      stories.order("last_comment_at DESC").where("published_at > ? AND score > ?", (tags.present? ? 5 : 2).days.ago, -5)
    end
  ...
  def self.active_eli5(time_ago)
    ...
    stories = if time_ago == "latest"
      ...
      stories.order("comments_count DESC").where("published_at > ?", time_ago).limit(6)
    else
      stories.order("last_comment_at DESC").where("published_at > ?", 5.days.ago).limit(3)
    end
  ...
  def self.seo_boostable(tag = nil, time_ago = 18.days.ago)
    ...
    if tag
      Article.published.cached_tagged_with(tag).order("organic_page_views_past_month_count DESC").where("score > ?", 8).where("published_at > ?", time_ago).limit(25).pluck(:path, :title, :comments_count, :created_at)
    else
      Article.published.order("organic_page_views_past_month_count DESC").limit(25).where("score > ?", 8).where("published_at > ?", time_ago).pluck(:path, :title, :comments_count, :created_at)
    end
  end
