class ClassifiedListing < ApplicationRecord
  ...
  def self.select_options_for_categories
    ...
    categories_available.keys.map do |key|
      [
        "#{categories_available[key][:name]} (#{ActionController::Base.helpers.pluralize(categories_available[key][:cost], "Credit")})",
        key,
      ]
    end
  end
  ...
  def self.categories_for_display
    ...
    categories_available.keys.map do |key|
      {
        slug: key,
        name: categories_available[key][:name],
      }
    end
  end
  ...
  def self.categories_available
    {
      ...
      "forhire" => {
        ...
        rules: "You are available for hire.",
      },
      "collabs" => {
        cost: 1,
        name: "Contributors/Collaborators Wanted",
        rules: "Projects looking for volunteers. Not job listings.",
      },
      "education" => {
        cost: 1,
      ...
      "mentors" => {
        cost: 1,
        name: "Offering Mentorship",
        rules: "You are available to mentor someone.",
      },
      "products" => {
        cost: 10,
        name: "Products/Tools",
        rules: "Must be available right now.",
      },
      "mentees" => {
        cost: 1,
        name: "Seeking a Mentor",
        rules: "You are looking for a mentor.",
      },
      "forsale" => {
        cost: 1,
        name: "Stuff for Sale",
