class GithubRepo < ApplicationRecord
  ...
  def self.find_or_create(params)
    repo = where(github_id_code: params[:github_id_code]).or(where(url: params[:url])).first_or_initialize
    repo.update(params)
    repo
  end
  ...
  def self.update_to_latest
    ...
    where("updated_at < ?", 1.day.ago).find_each do |repo|
      ...
      begin
        ...
        repo.update!(github_id_code: fetched_repo.id, name: fetched_repo.name, description: fetched_repo.description, language: fetched_repo.language, fork: fetched_repo.fork, bytes_size: fetched_repo.size, watchers_count: fetched_repo.watchers, stargazers_count: fetched_repo.stargazers_count, info_hash: fetched_repo.to_hash)
