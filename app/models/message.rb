class Message < ApplicationRecord
  ...
  validates(:message_markdown, presence: true, length: {
    maximum: 1024,
  })
  ...
  def preferred_user_color
    color_options = [
      user.bg_color_hex || "#000000",
      user.text_color_hex || "#000000",
    ]
  ...
  def send_push
    receiver_ids = chat_channel.chat_channel_memberships.where.not(user_id: user.id).pluck(:user_id)

    PushNotificationSubscription.where(user_id: receiver_ids).find_each do |sub|
      break if no_push_necessary?(sub)
      Webpush.payload_send(endpoint: sub.endpoint, message: ActionView::Base.full_sanitizer.sanitize(message_html), p256dh: sub.p256dh_key, auth: sub.auth_key, ttl: 24 * 60 * 60, vapid: {
  ...
  def update_all_has_unopened_messages_statuses
    chat_channel.chat_channel_memberships.where("last_opened_at < ?", 10.seconds.ago).where.not(user_id: user_id).update_all(has_unopened_messages: true)
  end
  ...
  def send_email_if_appropriate
    ...
    return if !chat_channel.direct? || recipient.updated_at > 2.hours.ago || recipient.chat_channel_memberships.order("last_opened_at DESC").first.last_opened_at > 18.hours.ago || chat_channel.last_message_at > 45.minutes.ago || recipient.email_connect_messages == false
