class Tag < ActsAsTaggableOn::Tag
  ...
  acts_as_followable
  resourcify
  NAMES = %w[beginners career computerscience git go java javascript react vue webassembly linux productivity python security webdev css php laravel opensource npm a11y ruby cpp dotnet swift testing devops vim kotlin rust elixir graphql blockchain scala vscode docker kubernetes aws android ios angular csharp typescript django rails clojure ubuntu elm gamedev flutter dart bash machinelearning sql].freeze
  attr_accessor(:tag_moderator_id, :remove_moderator_id)
  mount_uploader(:profile_image, ProfileImageUploader)
  mount_uploader(:social_image, ProfileImageUploader)
  validates(:text_color_hex, format: /\A#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\z/, allow_nil: true)
  validates(:bg_color_hex, format: /\A#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\z/, allow_nil: true)
  ...
  def calculate_hotness_score
    self.hotness_score = Article.tagged_with(name).where("articles.featured_number > ?", 7.days.ago.to_i).map do |a|
