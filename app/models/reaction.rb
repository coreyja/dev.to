class Reaction < ApplicationRecord
  ...
  belongs_to(:user)
  counter_culture(:reactable, column_name: proc { |model| model.points.positive? ? "positive_reactions_count" : "reactions_count" })
  counter_culture(:user)
  validates(:category, inclusion: {
    in: CATEGORIES,
  })
  validates(:reactable_type, inclusion: {
    in: %w[Comment Article],
  })
  validates(:status, inclusion: {
    in: %w[valid invalid confirmed],
  })
  validates(:user_id, uniqueness: {
    scope: %i[reactable_id reactable_type category],
  })
  ...
  class << self
    def count_for_article(id)
      Rails.cache.fetch("count_for_reactable-Article-#{id}", expires_in: 1.hour) do
        ...
        %w[like readinglist unicorn].map do |type|
          {
            category: type,
            count: reactions.where(category: type).size,
          }
        end
      end
    end
    ...
    def for_display(user)
      includes(:reactable).where(reactable_type: "Article", user: user).where("created_at > ?", 5.days.ago).select("distinct on (reactable_id) *").take(15)
    end
  ...
  def skip_notification_for?(receiver)
    points.negative? || (user_id == reactable.user_id) || (receiver.is_a?(User) && reactable.receive_notifications == false)
  end
